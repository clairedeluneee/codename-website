...
Create a new file on your mods folder: `data/shaders/crt.frag`. Paste the code from the shader into the file. Then, in `global.hx`, add the following code to the `onPostStateSwitch` function:
```hx
FlxG.camera.addShader(new CustomShader("example"));
```
...
Then, in our hx file:
```hx
var daShader = new CustomShader(`name-here`);
shader.blue = 0.5;
boyfriend.shader = daShader;
```
